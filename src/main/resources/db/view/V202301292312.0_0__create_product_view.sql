...
CASE WHEN B2_QATU < 4 THEN 0 ELSE B2_QATU END  AS ESTOQUE_PRODUTO,B2_ESTVTEX AS ESTOQUE_VTEX,
ISNULL((SELECT SUM(B2_QATU) FROM SB2010 B22 WHERE B2_LOCAL = '10' AND B2_FILIAL <> '0111' AND B22.D_E_L_E_T_ = '' AND B22.B2_COD = B1_COD),0) AS ESTOQUE_GERAL_ARMAZEM_10
FROM SB1010 (NOLOCK) B1 
INNER JOIN SB0010 (NOLOCK) B0 ON B0_COD = B1_COD
INNER JOIN SB2010 (NOLOCK) B2 ON B1_COD = B2_COD 
WHERE B1.D_E_L_E_T_ = '' AND B0.D_E_L_E_T_ = '' AND B2.D_E_L_E_T_ = '' AND B1_TIPO = 'ME' 
