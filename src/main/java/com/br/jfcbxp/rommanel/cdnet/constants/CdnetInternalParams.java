package com.br.jfcbxp.rommanel.cdnet.constants;

public final class CdnetInternalParams {

    public static final int PAGINATE_ROWS_DEFAULT = 500;
    public static final int PAGINATE_PAGE_DEFAULT = 0;
    public static final String PAGINATE_SORT_DIRECTION_DEFAULT = "DESC";
    public static final String PAGINATE_SORT_PRODUCT_INVENTORY_PROPERTIES_DEFAULT = "productCode";
    public static final String PAGINATE_SORT_SALE_PROPERTIES_DEFAULT = "document";
    public static final String ZONE_ID = "UTC-3";
    public static final String INTEGRATION_DATE_TIME_FORMAT = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'";
    public static final String INTEGRATION_DATE_FORMAT = "yyyyMMdd";
    public static final int PRODUCT_NOT_FOUND_ERROR_CODE = 400;
    public static final int PAGINATE_INVENTORY_ROWS_DEFAULT = 10000;
}
